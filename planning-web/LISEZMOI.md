...
- **Messages** (bouton 💬 sous « Nouveau créneau », touche D, ou « Plus » sur téléphone) : un fil « Toute l'équipe » et une conversation individuelle avec chaque collègue (lisible seulement par les deux personnes). Pastille des messages non lus, point vert « en ligne », chacun peut effacer ses propres messages.
- **Sons** (dossier `sons`) : `reunion.mp3` quand on enregistre une réunion, `connexion.mp3` quand un collègue se connecte pendant que le planning est ouvert, `notification.mp3` à la réception d'un message ou d'une nouvelle annonce. Pour changer un son, remplacez le fichier en gardant le même nom.
- **Couleurs au choix de chacun** (bouton 🎨 dans la barre latérale, ou « Plus » sur téléphone) : 5 palettes (Lagon par défaut, Bleu nuit, République, Mangrove, Graphite) et mode Automatique / Clair / Sombre. Le choix est enregistré dans le profil et retrouvé sur tous les appareils ; il ne change que son propre écran. Les activités créées par l'équipe reçoivent une couleur propre, adaptée à chaque palette. Tableau de la semaine : lignes alternées, pictogramme par activité, nombre de collègues en activité et absents sous chaque date, heures de la semaine avec jauge sous chaque nom.
